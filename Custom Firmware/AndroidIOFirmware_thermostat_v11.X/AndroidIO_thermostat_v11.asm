...
ADC_switch_pin	macro	portpinnr1, stat_adr1, portpinnr2, stat_adr2
	if_f_eq_l	stat_adr1,.3	; is pin 1 an ADC?
	 if_f_eq_l	stat_adr2,.2	; is pin 2 a digital output?
	  if_f_b_s	(stat_adr2+1),6	; Is pin 2 set as thermostat?
	   ; Thermostat code should be executed; now the actual code
	   ; first, measure the ADC
	   load_f_l	cmda1,.2	; read
	   load_f_l	cmda2,portpinnr1	; set pinnumber ADC
	   clrfb	(cmda2+1)
	   load_f_l	nofargs,.2	; two parameters
	   bsfb	cmdstat,0		; output only to txt_int
	   callp	cmd_exec_nocmdln	; read the ADC
	   bcfb	cmdstat,0		; set cmdstat back to output
	   ; Now we have the ADC value in txt_int
	   ; We compare the ADC-value with the value in satus 2 and 3 registers
	   ; If ADC value is higher, the temperature is lower, and the pin should
	   ; be switched on. Otherwise off. We put 0 or 1 in cmda3.
	   if_f16_gt_f16	txt_int, (stat_adr2+2)
	    load_f16_l16 cmda3, .1
	   else_
	    load_f16_l16 cmda3, .0
	   end_if
	   ; Now cmda3 contains the on or off value
	   ; We send this value to the pin
	   load_f_l	cmda1,.1	; write
	   load_f_l	cmda2,portpinnr2	; set pinnumber digtal output
	   clrfb	(cmda2+1)
	   load_f_l	nofargs,.3	; three parameters
	   callp	cmd_exec_nocmdln	; read the ADC
	   ; All done!
	  end_if ; (stat_adr2+1),6
	 end_if ; stat_adr2,.2
	end_if	; stat_adr1,.3
	endm
...
; B1 as digital output, with thermostat bit 6
;  and value .4850 (0x12F2) (20 degrees @ 10 measureents) in B12-B13:
	set_statusregister  .9, 0, 2
	set_statusregister  .9, 1, b'01000000'
	set_statusregister  .9, 2, 0xF2
	set_statusregister  .9, 3, 0x12
; B3 as ADC with 10 readings and sum:
	set_statusregister  .11, 0, 3
	set_statusregister  .11, 1, b'10'
	set_statusregister  .11, 2, .10
	return
	...
	end_if
	 ...
	 if_f_b_s	CNT_stat,7		; second flag set?
	  ...
	  if_f_b_s	Stat_X0,0		; show clock?
	   ...
	   write_char	.10
	  end_if ; Stat_X0,0
	  ; thermostat for ADC on B3 (11) and output on B1 (9)
	  ADC_switch_pin	.11, Stat_B3, .9, Stat_B1	
	 end_if ; CNT_stat,7
